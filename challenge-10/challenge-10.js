...
function calculator(op) {
	if(!isOperatorValid(op))
		return false;
	return function(x, y) {
		return (!(typeof x === 'number' && typeof y === 'number') ? false : operation[op](x, y));
	}
}
...
/*
Repita o PASSO 2 novamente, mas passando um operador inválido, para ver sef
...
if (inv) {
	number1 = 10;
	number2 = 20;
	console.log(showOperationMessage(operationSignal, number1, number2), inv(number1, number2));
} else {
	console.log(showErrorMessage(operationSignal));
}
